 ...
 */(function($, undefined){
	...
	var Datepicker = function(element, options){
		...
		if(this.o.setReservedDates) {

			this.timeStartHours = 9;
			if(this.o.timeStartHours)
				this.timeStartHours = this.o.timeStartHours

			this.timeEndHours = 18;
			if(this.o.timeEndHours)
				this.timeEndHours = this.o.timeEndHours

			this.resultReservedDates = [];
			this.setReservedDates(this.o.setReservedDates);
		}
...
//                for (var dateIndex in this.resultReservedDates[typeIndex]){}
					...
					for (var rIndex=0; rIndex < selected.length; rIndex++) {
						for (var i = 0; i < periods.length; i++) {
							if (periods[i].datetime >= selected[rIndex].rent_from && periods[i].datetime <= selected[rIndex].rent_to) {
								notSortedReservedDates[dateIndex].timePeriods[i].disabled = true;
							} else {
								disabled = false;
							}
						}
		...
		getClassNames: function(date){
			...
			if(this.o.setReservedDates){
                var findRes = this.findReserved(date);
				if(findRes.class){
					cls.push(findRes.class);
				}
			} else {
		...
		renderReservedInfo: function(dateIndex) {

			var html = '';
			var resInfo = this.picker.find('.reservedInfo');
			resInfo.text(html);

			this.lastClickDate = moment(dateIndex);
			...
			if(resReserved.class){
				...
				html = '<div class="result-title-day">'+this.lastClickDate.format("DD ") + this.monthNamesRod[this.lastClickDate.month()] + this.lastClickDate.format(" YYYY")+'</div>';
			...
			if (target.length === 1){
				switch (target[0].nodeName.toLowerCase()){
					...
					case 'td':
						...
						if(this.o.setReservedDates) {
							this.renderReservedInfo(this.viewDate);
						}
	...
	DPGlobal.template = '<div class="datepicker"><div class="reservedInfo"></div>'+
