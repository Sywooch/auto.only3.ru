...
//                '</div>' +
        ...
        if(typeof options.setReservedDates === 'object'){
            ...
            $.each(options.setReservedDates, function( ind, val ) {
                if(val.rent_from && val.rent_to){
                    if(moment(val.rent_from).isSame(moment(val.rent_to),'day')){
    ...
    DateRangePicker.prototype = {
        ...
        checkSelect: function () {
            ...
            if (this.realEndDate && this.realStartDate) {
                ...
                var startDate = moment.min(this.realStartDate, this.realEndDate);
                var endDate = moment.max(this.realStartDate, this.realEndDate);

                $.each(this.reservedDates, function (ind, val) {
                    if (val.rent_from && val.rent_to) {
                        if (startDate <= moment(val.rent_from) && moment(val.rent_to) <= endDate) {
                            resetDates = true;
                        }
                    }
                });
                ...
                if (resetDates) {
                    ...
                    if (this.endDate) {
                        this.container.find('input[name=daterangepicker_start]').val(startDate.format(this.locale.format));
                    } else {
...
//                    this.updateView();
        ...
        renderCalendar: function (side) {
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    var res = this.isInvalidDate(calendar[row][col], this);
                    if (res.class) {
                        classes.push(res.class);

                        if (res.class == 'full') {
                            classes.push('disabled');
                        } else if (res.class == 'reserved-right-half') {
                            this.rightHalfReservedDates.push(res.rentedDate);
                        } else {
                            this.leftHalfReservedDates.push(res.rentedDate);
                        }
        ...
        renderTimePicker: function(side) {
            ...
            console.log('selected=' +selected.format('YYYY-MM-DD H:mm:ss'));
            if(this.realStartDate)
            console.log('realStartDate=' +this.realStartDate.format('YYYY-MM-DD H:mm:ss'));

            if(this.realEndDate)
            console.log('realEndDate=' +this.realEndDate.format('YYYY-MM-DD H:mm:ss'));

            if(minDate)
                console.log('minDate=' +minDate.format('YYYY-MM-DD H:mm:ss'));

            if(maxDate)
                console.log('maxDate='+maxDate.format('YYYY-MM-DD H:mm:ss'));
            console.log('-----------------');
        ...
        renderTimePicker_b: function(side) {
            ...
            var chSelected = selected.format('DD.MM.YYYY');

            var fakeMaxDate = maxDate;
            $.each(this.rightHalfReservedDates, function(ind, val){
                if(chSelected == moment(val.rent_from).format('DD.MM.YYYY')){
                    if(moment(val.rent_from) > minDate) {
//                        fakeMaxDate = moment(val.rent_from);
//                        selected = fakeMaxDate;
//                        selected.add(-30, 'minutes');
                    }
                }
            });

            var fakeMinDate = minDate;
            $.each(this.leftHalfReservedDates, function(ind, val){
                if(chSelected == moment(val.rent_to).format('DD.MM.YYYY')){
//                    fakeMinDate = moment(val.rent_to);
//                    selected = fakeMinDate;
                }
            });
            ...
            for (var i = start; i <= end; i++) {
                ...
                if (minDate && time.minute(59).isBefore(fakeMinDate) )
                    disabled = true;
                if (maxDate && time.minute(0).isAfter(fakeMaxDate))
                    disabled = true;
            ...
            for (var i = 0; i < 60; i += this.timePickerIncrement) {
                ...
                var disabled = false;
                if (minDate && time.second(59).isBefore(fakeMinDate))
                    disabled = true;
                if (maxDate && time.second(0).isAfter(fakeMaxDate))
                    disabled = true;
        ...
        clickDate: function(e) {
            ...
            if(!this.realEndDate){
                var startDateRes = this.realStartDate;
                var endDateRes = this.realStartDate.startOf('day');
            } else {
                var startDateRes = moment.min(this.realStartDate, this.realEndDate);
                var endDateRes = moment.max(this.realStartDate, this.realEndDate);

                this.realStartDate = startDateRes;
                this.realEndDate = endDateRes;

            }

            this.setStartDate(startDateRes);
            this.setEndDate(endDateRes);
