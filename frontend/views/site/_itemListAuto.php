...
$urlReserve = Url::toRoute(['/cars/reserve-a-car', 'id' => $model->id]);
$urlReserve = Url::toRoute(['cars/reserve-a-car',
    'city_url' => $model->account->city->trans,
    'slug_url' => $model->account->slug_url,
    'slug_url_system' => $model->slug_url]);

//$urlSalon = Url::toRoute(['/cars/our-cars', 'id' => $model->account_id]);
//html::img($model->getThumbImage($model->photo, 210, 158))
?>

<div  class="it-1">
    <a href="<?=$urlReserve;?>"><?=html::img($model->getThumbImage($model->photo, 159, 150))?></a>
</div>
    ...
    <div style="position: relative;">
        ...
        <div class="it-2-head">
         ...
         <div class="it-2-salon">
            <a  href="<?=$model->account->UrlSalon;?>">Автопрокат <?=html::encode($model->account->username)?></a>
        </div>
    </div>
